...
def shape_to_mask(
    ...
    mask = PIL.Image.fromarray(mask)  # 数组转图片
    ...
    if shape_type == "circle":
        ...
        draw.polygon(xy=xy, outline=1, fill=1) # xy点序列，outline轮廓颜色，fill是填充颜色
...
def shapes_to_label(img_shape, shapes, label_name_to_value):
    ...
    for shape in shapes:
        ...
        if group_id is None:
            group_id = uuid.uuid1()  # 从主机ID、序列号和当前时间生成UUID
        shape_type = shape.get("shape_type", None)
        # 用dict.get()的方式，没有这一项就返回none，而不是报错
